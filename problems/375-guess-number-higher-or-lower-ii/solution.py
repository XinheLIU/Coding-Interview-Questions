...
class Solution:
    def getMoneyAmount(self, n: int) -> int:
        # dp[i][j] is the amount to pay to search in range (i, j)
        dp = [[0 for _ in range(n + 1)] for _ in range(n + 1)]
        # dp[i][j] = min_(pivot in (i,j)) pivot + max(cost(i, pivot),cost(pivot, j)) 
        for len in range(2, n + 1):
            for start in range(1, n - len + 2):
                min_ret = float('inf')
                 # only search in (i + (len - 1) / 2, j) for symmetry
                for pivot in range(start + (len - 1)// 2, start + len - 1):
                    ret = pivot + max(dp[start][pivot - 1], dp[pivot + 1][start + len - 1])
                    min_ret = min(ret, min_ret)
                dp[start][start + len - 1] = min_ret
        return dp[1][n]
