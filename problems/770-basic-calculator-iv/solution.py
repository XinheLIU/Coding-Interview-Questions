...
class Poly(collections.Counter):
    ...
    def to_list(self):
        ...
        """
        return ["*".join((str(v),) + k)
                for k, v in sorted(self.items(), key=lambda x: (-len(x[0]), x[0], x[1])) if v]
...
class Solution(object):
    def basicCalculatorIV(self, expression: str, evalvars: List[str], evalints: List[int]) -> List[str]:
        ...
        def combine(left, right, symbol):
            ...
            the binary operator represented by 
            symbol to left and right.
            """
        ...
        def make(expr):
            """ makes a new Poly 
            represented by either the constant 
            or free variable specified by expr.
            """
            ans = Poly()
            if expr.isdigit():
                ans.update({(): int(expr)})
