...
def resolved_chapter(fm: dict, readme: Path) -> tuple[str, str]:
    """Return (leaf chapter id, section), honouring a leaf `chapter:` override.
    ...
    if override:
        if override not in taxonomy.LEAF_CHAPTERS_BY_ID:
            allowed = ", ".join(c.id for c in taxonomy.LEAF_CHAPTERS)
            raise ValueError(f"{readme}: chapter override must be a leaf id: {allowed}")
        if override != chapter:
            # The override moves the problem; keep a section that exists in the
            # destination leaf so the index can still group it.
            chapter = override
            override_topics = [topic for topic in topics
                               if taxonomy.TOPICS[topic].chapter == chapter]
            if override_topics:
                _, section = taxonomy.classify(override_topics)
            else:
...
def main() -> None:
    ...
    # Emit generated sidebar fragments so taxonomy order has one source of truth.
    ...
    def chapter_count(chapter_id: str) -> int:
        return sum(len(by_chapter.get(child.id) or [])
                   for child in taxonomy.descendants_of(chapter_id))

    chapter_sidebar = []
    for meta in taxonomy.META_CHAPTERS:
        chapter_sidebar.append({
            "text": meta.title,
            "link": meta.page,
            "items": [
                {"text": f"{leaf.title} ({len(by_chapter.get(leaf.id) or [])})",
                 "link": leaf.page}
                for leaf in taxonomy.children_of(meta.id)
            ],
        })
    chapter_sidebar_out = REPO / ".vitepress" / "sidebar-chapters.json"
    chapter_sidebar_out.write_text(json.dumps(chapter_sidebar, indent=2) + "\n")
    print(f"Wrote {chapter_sidebar_out.relative_to(REPO)} (imported by config.mts).")

    graph = {
        # `meta_chapters` drives the homepage map. `chapters` includes both
        # levels so leaf pages can render their own counts and sections.
        "meta_chapters": [
            {
                "id": chapter.id,
                "title": chapter.title,
                ...
                "parent": None,
                "level": 1,
                "count": chapter_count(chapter.id),
                "children": [child.id for child in taxonomy.children_of(chapter.id)],
            }
            for chapter in taxonomy.META_CHAPTERS
        ],
        "chapters": [
            {
                "id": chapter.id,
                ...
                "level": 1 if chapter.parent is None else 2,
                "count": chapter_count(chapter.id),
                "children": [child.id for child in taxonomy.children_of(chapter.id)],
                "sections": [
                    {
        ...
        "leaf_chapters": [chapter.id for chapter in taxonomy.LEAF_CHAPTERS],
    ...
    print(f"Wrote {graph_out.relative_to(REPO)} ({len(graph['edges'])} relations, "
          f"{len(graph['meta_chapters'])} meta-chapters, "
          f"{len(graph['leaf_chapters'])} leaf chapters).")
